...
class WPClickmeter {
	...
	function show_clickmeter_column($name) {
		...
		if($name=='views'){
			...
			if($pixel_id!=NULL){
				...
				echo '<a target="blank" href="http://mybeta.clickmeter.com/go?val='.$boGoVal.'&returnUrl=%2FLinks%3FlinkId%3D'.$pixel_id.'">'.$views.'</a><br>';
			}else{
				delete_post_meta($post->ID, 'trackingpixel_views', $views);
				update_post_meta($post->ID, 'trackingpixel_views', $views);
				echo $views.'<br>';
		...
		if($name=='conversion1'){
			...
			if($conversion1_id!=NULL){
				...
				echo '<a target="blank" href="http://mybeta.clickmeter.com/go?val='.$boGoVal.'&returnUrl=%2FConversions%3FconversionId%3D'.$conversion1_id.'">'.$conversion1_count.'</a><br>';
			}else{
				delete_post_meta($post->ID, 'conversion1_count', $conversion1_count);
				update_post_meta($post->ID, 'conversion1_count', $conversion1_count);
		...
		if($name=='conversion2'){
			...
			if($conversion2_id!=NULL){
				...
				echo '<a target="blank" href="http://mybeta.clickmeter.com/go?val='.$boGoVal.'&returnUrl=%2FConversions%3FconversionId%3D'.$conversion2_id.'">'.$conversion2_count.'</a><br>';
			}else{
				delete_post_meta($post->ID, 'conversion2_count', $conversion2_count);
				update_post_meta($post->ID, 'conversion2_count', $conversion2_count);
	...
	*/
	...
	static function create_tracking_pixel($post_id, $tracking_pixel_id="", $update=false){
		...
		if($update==true && $tracking_pixel_id!=""){
			...
			if ( !wp_is_post_revision( $post_id ) ){
				...
				remove_action( 'edit_post', array(__CLASS__, 'clickmeter_edit_meta_box_data' ));
				...
				if(!empty($post->post_content)){
					...
					//Check for remaining noscripts
					$noscriptTags = $doc->getElementsByTagName('noscript');
					foreach ($noscriptTags as $noscript) {
						//if(preg_match("/".$pixel_name."/",$noscript->getElementsByTagName("img")->item(0)->getAttribute("src"))){
							$remove[] = $noscript;
						//}
					}
					foreach ($remove as $item){
						//WPClickmeter::store_option("clickmeter_debug_content", $item->parentNode);
					...
					//WPClickmeter::store_option("clickmeter_debug",$new_content );

					// update the post, which calls save_post again
					if(empty($new_content)){
						$new_content =  $tracking_pixel;	
					}else{
						$new_content = $new_content . $tracking_pixel;	
					}
					$modified_post = array(
					'ID'           => $post_id,
			...
			if ( !wp_is_post_revision( $post_id ) ){
				...
				// unhook this function so it doesn't loop infinitely
				remove_action('save_post', array(__CLASS__, 'clickmeter_save_meta_box_data'));
			...
			//$info = array($post_content,$trackingCode,$tracking_pixel);
			//WPClickmeter::store_option("clickmeter_debug", $info);
			...
			if($created_pixel_id!=null){
				WPClickmeter::store_pixel($post_id, $created_pixel_id, $pixel_name, $trackingCode);	
			}
		}
	}
	...
	static function store_pixel($post_id, $pixel_id, $pixel_name, $tracking_code){
		global $wpdb;
	...
	static function store_link($post_id, $link_id, $link_name, $link_rid, $campaign_id, $campaign_name, $trackingCode, $url, $domain_id, $is_post, $is_redirection_link){
		global $wpdb;
...
function ajax_create_tl() {
	...
	if(($link_cloak_flag=="true") || $link_cloak_flag==1){
		$body=array('type'=> 0,
			...
			'name'=> $url_name,
			'typeTL'=>array('domainId'=> $domainId,'url'=> $url, "hideUrl"=> "true", 'redirectType'=> $redirection_type)
		);	
	}else{
		$body=array('type'=> 0,
			'title'=> $friendly_name,
	...
	if($created_link_id!=null){
		WPClickmeter::store_link($post_id, $created_link_id, $tmp_name, $link_rid ,$default_campaignId, $campaign_name, $trackingCode, $url, $domainId, $is_post, $redirection_flag);
	}
	...
	if($pixels_flag==1){
            ...
            foreach ($inclusion_list as $post_id) {
                ...
                //Look for already existent pixels in clickmeter for actual campaign with the same title of the post
           		if($pixel_data!=null){
                    ...
                    $trackingCode = $pixel_data[tracking_code];
                    $tracking_pixel ="<div id='clkmtr_tracking_pixel'>
                    	<!--ClickMeter.com WordPress tracking: ".$post->post_title." -->
                        <script type='text/javascript'>
                        var ClickMeter_pixel_url = '".$trackingCode."';
                        </script>
                        <script type='text/javascript' id='cmpixelscript' src='https://www.clickmeter.com/js/pixel.js'></script>
        ...
        if(!empty($toCreateList)){
            ...
            foreach ($toCreateList as $post_id) {
                ...
                if($conversion1_id!=null) $track_pix["firstConversionId"] = $conversion1_id;
                if($conversion2_id!=null) $track_pix["secondConversionId"] = $conversion2_id;
                $body["list"][] = $track_pix;
                ...
                if($i==sizeof($toCreateList) && !empty($body)){
	                ...
	                foreach ($json_output[results] as $result) {
	                    $tracking_pixel = getTrackingPixel($result, $result[entityData][title]);
						...
						WPClickmeter::store_pixel($pid, $created_pixel_id, $created_pixel_name, $trackingCode);

	                    //add tracking pixel to post
	                    if(empty($postToChange->post_content)){
	                        $new_content =  $tracking_pixel;    
	                    }else{
	                        $new_content = $postToChange->post_content . $tracking_pixel;   
	                    }
	                    $modified_post = array(
	                    'ID'           => $postToChange->ID,
	                    'post_content' => $new_content
...
function TP_create_conversion() {
	...
	if($conversion1_id!=null){
		...
		$conversion1_id = $conversionId;
		$conversion1_name = $conversion_name;
	}

    //Update tracking pixel in batch mode
    if(!empty($pixels_list)){
		foreach ($pixels_list as $pixel) {
			$body=array('action'=>'add','id'=>$pixel[id] );
